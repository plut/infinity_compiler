...
pub trait Resource: SqlRow {
	...
	fn insert_subresources(&self, db: &Connection, node: &mut Self::StatementNode<'_>, primary: impl ToSql+Copy)->Result<()>;
	// Provided functions
	...
	fn insert_as_topresource(&self, db: &Connection, node: &mut Self::StatementNode<'_>, resref: Self::Primary)->Result<usize> {
		let n = self.bind_execute1(node.deref_mut(), resref)?;
		if n == 0 {
			warn!("skipped inserting {resref}");
			return Ok(n)
		}
		self.insert_subresources(db, node, resref)?;
		Ok(1)
	}
	...
	fn insert_as_subresource(&self, db: &Connection, node: &mut Self::StatementNode<'_>, parent: impl ToSql, position: usize)->Result<()> {
...
pub trait ResourceIO: Resource<Primary=Resref> {
	// Required methods
	...
	// Provided methods
	fn load_from_handle(db: &Connection, node: &mut Self::StatementNode<'_>, resref: Resref,
