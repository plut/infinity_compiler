...
pub trait ResourceTree: SqlRow {
	...
	fn select_subresources<'a,'b,'c>(&'a mut self, branches: &'b mut Self::StatementForest<'c>,
...
// 	fn read_rows<'a:'b,'b,'c: 'a>(tree: &'a mut Tree<Self::StatementForest<'c>>, params: impl rusqlite::Params)->Result<RecursiveRows<'a,'b,'c,Self>> {
// 		todo!()
// 		let rows = tree.content.query(params)?;
// 		Ok(RecursiveRows{ rows: rows.into(),
// 			branches: &mut tree.branches })
// 	}
	fn collect_all<'a,'b>(tree: &'a mut Tree<Self::StatementForest<'b>>, params: impl rusqlite::Params)->Result<Vec<Self>> {
...
#[allow(missing_debug_implementations)]
pub struct RecursiveRows<'a,'b,'c, T: ResourceTree> {
	rows: rusqlite::Rows<'a>,
	branches: &'b mut T::StatementForest<'c>,
}
impl<'a,'b:'c,'c, T: ResourceTree> Iterator for RecursiveRows<'a,'b,'c,T> {
	type Item = Result<(T::Primary, T)>;
	fn next(&mut self)->Option<Result<(T::Primary,T)>> {
		use crate::sql_rows::FromSqlRow;
		loop {
			let row = match self.rows.next() {
				Ok(Some(row)) => row,
			...
			let (primary, mut resource) = match <(T::Primary,T)>::from_row(row) {
				Err(e) => continue,
				Ok(x) => x,
			};
			resource.select_subresources(self.branches, primary);
			return Some(Ok((primary, resource)))
		}
	}
}
