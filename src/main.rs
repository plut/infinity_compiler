...
use crate::restypes::{AllTables,RootForest};
...
pub trait RecResource: SqlRow {
	...
	/// The type of the primary key for this resource:
	/// either Resref for top-level or i64 for sub-resources.
	type Primary9: ToSql + FromSql + Display + Copy;
	/// Always `FooNode<Statement<'a>>`.
	type StatementNode<'a>: DerefMut<Target=Statement<'a>> + 'a;
	/// Given an object inserted into the database,
	/// insert its sub-resources. Generated by macro
	/// and mutually recursive with `(subresources)::insert_as_subresource9`.
	fn insert_subresources9(&self, db: &Connection, node: &mut Self::StatementNode<'_>, primary: impl ToSql+Copy)->Result<()>;
	// Provided functions
	/// Inserts a top-level resource.
	/// This differs from subresources in that the primary key (resref) is
	/// known.
	...
	/// This is the entry point called when populating the database;
	/// this function is not called recursively.
	fn insert_as_topresource9(&self, db: &Connection, node: &mut Self::StatementNode<'_>, resref: Self::Primary9)->Result<usize> {
		let n = self.bind_execute1(node.deref_mut(), resref)?;
		if n == 0 {
			warn!("skipped inserting {resref}");
			return Ok(n)
		}
		self.insert_subresources9(db, node, resref)?;
		Ok(1)
	}
	/// Inserts a sub-resource.
	///
	...
	///
	/// This function is mutually recursive with
	/// `Self::insert_subresources9` (derived from macro).
	fn insert_as_subresource9(&self, db: &Connection, node: &mut Self::StatementNode<'_>, parent: impl ToSql, position: usize)->Result<()> {
		self.bind_execute2(node.deref_mut(), parent, position)?;
		let primary = db.last_insert_rowid();
		self.insert_subresources9(db, node, primary)?;
		Ok(())
	}
	fn select_subresources9(&mut self, node: &mut Self::StatementNode<'_>,
				primary: Self::Primary9)->Result<()>;
	fn collect_all9(node: &mut Self::StatementNode<'_>, params: impl rusqlite::Params)->Result<Vec<Self>> {
		todo!()
...
// 	/// `(Primary9, Self)`.
// 	fn iter_rows<'a>(s: &'a mut Statement<'_>, params: impl rusqlite::Params)->Result<TypedRows<'a,(Self::Primary9,Self)>> {
// 	de
...
pub trait ResourceIO: RecResource<Primary9=Resref> + ResourceTree {
	// Required methods
...
impl<T: RecResource<Primary9=Resref>+ResourceTree<Primary=Resref>+ResourceIO> crate::database::LoadResource for T {
